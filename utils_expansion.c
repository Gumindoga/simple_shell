...
/**
 * expand_var - Expands the variables in a command line
...
/**
 * exp_alias - Expands and alias in a command line
 * @str: Command line
 * @data: Program data
	...
	while (*str)
	{
		if (*str == ' ')
		{
			...
			if (alias)
			{
				idx -= i;
				line[idx] = '\0';
				_strcat(line, ret ? exp_alias(alias->value, 0, data) : alias->value);
